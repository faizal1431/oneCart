...
function Nav() {
    let { getCurrentUser, userData, logout } = useContext(userDataContext) // ✅ using logout from context
    ...
    const handleLogout = async () => {
        try {
            await logout() // ✅ FIXED: uses logout from UserContext (POST + clears userData)
            setShowProfile(false)
            navigate("/login")
            toast.success("Logged out successfully") // ✅ added toast feedback
        } catch (error) {
            console.log(error)
        ...
        <div className='w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex  items-center justify-between px-[30px] shadow-md shadow-black '>
            ...
            {showProfile && <div className='absolute w-[220px] h-[150px] bg-[#000000d7] top-[110%] right-[4%] border-[1px] border-[#aaa9a9] rounded-[10px] z-10'>
                <ul className='w-[100%] h-[100%] flex items-start justify-around flex-col text-[17px] py-[10px] text-[white]'>
                    ...
                    {userData && <li className='w-[100%] hover:bg-[#2f2f2f]  px-[15px] py-[10px] cursor-pointer' onClick={handleLogout}>LogOut</li>} {/* ✅ simplified onClick */}
